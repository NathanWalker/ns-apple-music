    ...
    <ScrollView iosContentInsetAdjustmentBehavior="automatic">
      <StackLayout class="pb-32">
        ...
        <GridLayout class="px-5 pt-3">
          ...
          >
            ...
            <StackLayout verticalAlignment="bottom" class="p-4" sharedTransitionTag="albumInfo">
              <Label text="FEATURED ALBUM" class="text-xs font-bold" color="#ffffff" opacity="0.85"></Label>
              <Label [text]="topPicks[0].title" class="text-3xl font-bold" color="#ffffff"></Label>
              <Label [text]="topPicks[0].artist" class="text-base" color="#ffffff" opacity="0.85"></Label>
            </StackLayout>
          </GridLayout>
        </GridLayout>
        ...
        <StackLayout class="pt-7">
          <Label text="Top Picks" class="text-2xl font-bold px-5" [color]="fg()"></Label>
          <ScrollView orientation="horizontal">
            <StackLayout orientation="horizontal" class="px-5 pt-3">
              @for (album of topPicksTail; track album.id) {
                <StackLayout class="mr-4" width="200" (tap)="open(album, TOP_PICKS)">
                  <ImageCacheIt
                    [src]="album.artwork"
                    width="200"
                    height="200"
                  ...
                  ></ImageCacheIt>
                  <Label [text]="album.title" class="text-base pt-2" [color]="fg()" maxLines="1" textWrap="false"></Label>
                  <Label [text]="album.artist" class="text-sm" [color]="muted()" maxLines="1" textWrap="false"></Label>
                </StackLayout>
              }
            </StackLayout>
          </ScrollView>
        ...
        <StackLayout class="pt-7">
          <Label text="Recently Played" class="text-2xl font-bold px-5" [color]="fg()"></Label>
          <StackLayout class="px-5 pt-3">
            @for (album of recentlyPlayed; track album.id) {
              <GridLayout columns="64, *, auto" rows="auto" class="py-2" (tap)="open(album, RECENTLY_PLAYED)">
                <ImageCacheIt
                  col="0"
                  [src]="album.artwork"
                  width="56"
                ...
                <StackLayout col="1" verticalAlignment="center" class="pl-3">
                  <Label [text]="album.title" class="text-base" [color]="fg()" maxLines="1" textWrap="false"></Label>
                  <Label [text]="album.artist" class="text-sm" [color]="muted()" maxLines="1" textWrap="false"></Label>
                </StackLayout>
        ...
        <!-- Made For You -->
        <StackLayout class="pt-7">
          <Label text="Made For You" class="text-2xl font-bold px-5" [color]="fg()"></Label>
          <ScrollView orientation="horizontal">
            <StackLayout orientation="horizontal" class="px-5 pt-3">
              @for (album of madeForYou; track album.id) {
                <StackLayout class="mr-4" width="160" (tap)="open(album, MADE_FOR_YOU)">
                  <ImageCacheIt
                    [src]="album.artwork"
                    width="160"
                    height="160"
                  ...
                  ></ImageCacheIt>
                  <Label [text]="album.title" class="text-sm pt-2" [color]="fg()" maxLines="1" textWrap="false"></Label>
                  <Label [text]="album.artist" class="text-xs" [color]="muted()" maxLines="1" textWrap="false"></Label>
                </StackLayout>
              }
            </StackLayout>
          </ScrollView>
