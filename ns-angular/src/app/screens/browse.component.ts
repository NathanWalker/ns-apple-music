    ...
    <ScrollView iosContentInsetAdjustmentBehavior="automatic">
      <StackLayout class="pb-32">
        <StackLayout class="px-5 pt-3">
          <GridLayout rows="*" columns="*" borderRadius="14" class="overflow-hidden" (tap)="open(hero, HERO)">
            <ImageCacheIt
              [src]="hero.artwork"
              stretch="aspectFill"
              height="320"
            ...
            <StackLayout verticalAlignment="bottom" class="p-4">
              <Label text="NEW RELEASE" class="text-xs font-bold" color="#ffffff" opacity="0.85"></Label>
              <Label [text]="hero.title" class="text-3xl font-bold" color="#ffffff"></Label>
              <Label [text]="hero.artist" class="text-base" color="#ffffff" opacity="0.85"></Label>
            </StackLayout>
          </GridLayout>
        </StackLayout>

        <Label text="Hot Tracks" class="text-2xl font-bold px-5 pt-7 pb-2" [color]="fg()"></Label>
        @for (pair of pairs; track pair[0].id) {
          <GridLayout columns="*, *" rows="auto" class="px-5 pb-3">
            ...
            @if (pair[1]; as second) {
              <StackLayout col="1" class="pl-2" (tap)="open(second, HOT_TRACKS)">
                <ImageCacheIt
                  [src]="second.artwork"
                  stretch="aspectFill"
                  height="170"
